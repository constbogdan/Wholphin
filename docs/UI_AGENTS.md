...
For optional fork-added presentation, prefer removing enhanced state at the ViewModel/projection boundary instead of scattering preference checks through cards. Upstream Discover and its Seerr request/availability UI remain base Wholphin behavior. When enhanced integrity is disabled, its `Incomplete` badge/subtitle disappears because no integrity presentation state is exposed; the shared card itself should not own feature-policy decisions.
