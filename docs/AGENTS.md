...
Pull requests targeting `main`, including upstream-sync pull requests, receive the required fork-owned `CI / Full validation` check. The job deterministically classifies the complete PR range: proven non-Android changes run changed-range pre-commit and offline workflow/Python checks; normal Android changes add mapped Kotlin/JVM coverage; unknown or sensitive build, release, signing, updater, persistence, identity, or CI boundaries retain Full default-debug validation. After a merge to protected `main`, the same job always runs repository-wide pre-commit and authenticates whether successful required PR Full validated the exact final `main^{tree}`. Exact equivalence reuses that Debug Full evidence; direct changes, non-Full PR validation, missing/ambiguous evidence, tree differences, and all uncertainty run the complete default-debug graph on main. The conditional I02 Release assembly remains authoritative on final main and is never reused from a PR artifact. Manual upstream conflict recovery still requires local Standard with meaningful focused filters followed by Full. CI is not a replacement for semantic review or Android TV visual, focus, and integration testing.
...
user explicitly authorizes publication
        ...
        -> reuse PR Full only on equality; otherwise run Full on main
```
