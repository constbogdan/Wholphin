...
- [ ] **I06 — IMPLEMENT — Refine Upstream Sync journaling, outcome semantics, and cadence** (Items 23–26, 62–64, 108–119, 128–131, 134, 142, 143)
  - Status: **IMPLEMENTED / OFFLINE VALIDATED; NATURAL HOSTED ACCEPTANCE PENDING.** Trusted policy v1 classifies exact automation paths as FOLLOW, REVIEW or DOWNSTREAM-OWNED; unknown automation defaults to REVIEW. Every path retains status/blob/ownership evidence, including excluded upstream evolution.
  - Outcomes distinguish `no_delta`, `observed_excluded`, ready/review/semantic candidates, existing/created normal or Draft PRs, `blocked`, `infrastructure_error` and `publication_error`. Security/provenance uncertainty still fails closed.
  - Attention episodes are keyed independently of title and whole downstream HEAD by policy version plus attention paths, ownership/status, downstream path blob identities and textual-conflict signature. Unrelated downstream movement therefore reuses the same open Issue and Draft PR. Exact upstream/run identities remain observation history; continued upstream movement in the same area updates that episode rather than overwriting the Draft branch. Closed/rejected PRs and human-edited exact candidates still fail closed and are never reopened or force-pushed.
  - Every observation of the same unresolved episode updates first/latest observed time, observation count and latest run URL on one prioritized Issue. Risk, integration debt, age and escalation are separate deterministic dimensions; title changes never affect deduplication. Existing `risk: *`, `debt: *` and conditional `attention` labels are replaced as metrics evolve; absent labels are reported for one-time external creation, never created by broader App authority.
  - `no_delta` and DOWNSTREAM-OWNED-only outcomes retain workflow artifact/summary evidence but create no attention Issue. Clean FOLLOW creates a historical journal, links it to the normal candidate PR and closes it after handoff. REVIEW and textual conflict create/reuse a Draft PR and keep the linked prioritized Issue open.
  - Final Draft-vs-normal review: Draft remains an intentional safety invariant under the current protected-`main` ruleset, which requires PR integration and `CI / Full validation` but zero formal approvals. The Issue owns prioritization/history and CI owns objective validation; neither blocks merge. Draft therefore supplies the enforced not-semantically-ready gate until a human deliberately marks the resolved candidate ready. Earlier CodeRabbit or other reviewer engagement is secondary and must not weaken merge safety.
  - Conflict Drafts use an exact downstream parent, safe non-conflicting changes, downstream conflict-path bytes and deterministic context JSON; they contain no markers and claim no upstream ancestry. No force push, automatic resolution, reopening or merge exists.
  - Quiet-upstream boundary: downstream Issue/PR bodies keep linked Draft/Issue/run navigation but render upstream PR numbers, short commit identities and attention filenames as inert sanitized text. Rich clickable upstream PR/commit/blob and Mosaic blob navigation lives only in the Actions summary. The retained JSON artifact carries complete exact URL/SHA/ref/object provenance. This prevents normal downstream conversations from broadcasting cross-references or mentions into upstream while preserving navigation and evidence.
  - Imported upstream commits and their original messages remain untouched in normal FOLLOW ancestry. Whether GitHub re-emits references when an already-known commit becomes reachable in a fork remains a separate platform question; ancestry and messages are not rewritten for hypothetical suppression.
  - Local resolution entry point: `.\scripts\resolve-upstream.ps1` discovers durable open episodes and always presents a selector, while `-Pr <N>` supports direct selection. For multiple candidates that already exist, deterministic semantic-path overlap plus upstream/downstream ancestry permits parallel independent candidates but serializes each overlap chain; stale or ambiguous evidence fails closed. The hosted publisher's existing one-open-sync-PR creation guard remains unchanged and requires a separate explicit policy decision before hosted operation can create concurrent independent candidates. First use verifies/checks out and writes `.logs/upstream-resolution/pr-<N>/codex-prompt.md`; re-entry refreshes state, audits resolution scope, derives/verifies I03 JVM filters and requires explicit default-No `Ready to PUSH?` authorization before delegating unchanged filters to prepare-pr for the same Draft. Fixture coverage includes discovery, dependency/order/re-evaluation, interactive/direct selection, refusals, evidence binding, scope/filter derivation, default-No/explicit-Yes delegation and zero automatic Ready/merge/destructive Git behavior.
  - Resolver handoff follow-up: terminal output now points concisely to the authoritative prompt file instead of printing it in full. That prompt requires exact meaningful JVM filters justified by the completed semantic resolution, or an explicit missing-test recommendation; resolver-side pre-resolution guessing and prepare-pr enforcement remain unchanged.
  - Merged-episode follow-up: a canonical merged candidate event authenticates exact PR/branch/episode/merge linkage, updates and closes only its linked journal with a deterministic terminal record, preserves unrelated labels and is idempotent. Ambiguous or mismatched linkage fails closed; the finalizer has no Contents write and uses no App token.
  - Ancestry assessment: PR #36 semantically integrated upstream `4a567cb`, `0b995b5` and `9c56965`, but none is an ancestor of current `origin/main`. Clearing that behind count safely would require a separately approved tree-preserving two-parent merge commit after resolution/validation, with the exact upstream tip as second parent and the reviewed resolved tree unchanged. Cherry-pick, graft/rebase and blanket-`ours` approaches are rejected. Current I06 intentionally retains the no-fabricated-ancestry behavior.
  - PR #33 read-only acceptance evidence: Draft #33 / journal #32 projected Medium Risk, High Debt, 1h Age, Attention, three attention paths, ten clean paths and failed `CI / Full validation`. Its head was one commit ahead and zero behind current `main`, confirming prepare-pr can reuse the same open PR through ordinary fast-forward publication after semantic resolution and required upstream-sync validation; later base ancestry drift remains a deliberate refusal.
  - Schedule is fixed UTC `0 6,15,21 * * *`, with DST limitations explicit; manual dispatch remains. Existing token permissions are unchanged and Repo Intelligence is unchanged.
  - Offline evidence: all 55 hosted-sync tests pass. They cover exact merged-journal closure/idempotency, three observations reusing one Issue/Draft, unrelated downstream movement, continued same-area upstream movement, priority progression, label replacement/missing labels, clean/attention journal lifecycles, quiet hostile-input Issue/PR surfaces, rich Actions navigation and retained exact artifact provenance, in addition to all prior ownership, conflict, credential, drift and no-force cases.
  - Regression evidence: the complete offline tooling suite passes 193 tests with one existing Windows executable-bit portability skip, preserving I01–I05 classification, delivery, signing, recovery, Stable, validation and updater contracts.
