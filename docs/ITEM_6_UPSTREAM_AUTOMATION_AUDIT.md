...
| [mosaic-development-release.yml](../.github/workflows/mosaic-development-release.yml) / `Mosaic development release` | Completed CI on main or explicit expected-SHA dispatch; guards authenticate successful protected-main push/current SHA. Classify, sign exact CI input, publish; zero Gradle. | Default contents/actions read; Environment-bound `sign` is read-only; `publish` alone contents write. | Signed seven-day artifact, immutable `downstream-build-N`, rolling `develop`; APK alias plus manifest. Eligibility summary is useful; signer summary incorrectly describes production as exercise. | **DOWNSTREAM-OWNED** |
| [mosaic-development-resume.yml](../.github/workflows/mosaic-development-resume.yml) / `Mosaic development resume` | Manual approved tooling SHA, source SHA, immutable artifact ID and unsigned/signed checkpoint. Sign unsigned or reverify signed checkpoint, then publish without build. | Contents/actions read until publisher; Environment only on unsigned signer; publisher contents write. | Signed or verified recovery artifacts, seven days; original identity retained; shared Development publisher. Exercise wording and source/tooling SHA distinction need presentation review. | **DOWNSTREAM-OWNED** |
