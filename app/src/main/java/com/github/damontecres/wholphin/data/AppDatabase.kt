...
@Database(
    entities = [
        ...
        SeerrUser::class,
        RememberedTab::class,
        SeasonIntegrityExpectationCache::class,
    ],
    version = 36,
    exportSchema = true,
    autoMigrations = [
...
abstract class AppDatabase : RoomDatabase() {
    ...
    abstract fun seasonIntegrityDao(): SeasonIntegrityDao
}
...
class Converters {
    ...
    @TypeConverter
    fun convertIntSet(values: Set<Int>?): String? = values?.sorted()?.joinToString(",")

    @TypeConverter
    fun convertIntSet(value: String?): Set<Int>? =
        value
